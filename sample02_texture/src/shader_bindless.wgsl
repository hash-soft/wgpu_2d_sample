// wgsl-analyzer使用時
// wgpu拡張機能はIDE上でエラー扱いされる

enable wgpu_binding_array;  // wgpu拡張機能
...
// Fragment shader
@group(0) @binding(0) var textures: binding_array<texture_2d<f32>, 3>;  // wgpu拡張機能
@group(0) @binding(1) var s_diffuse: sampler;

@fragment
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    return textureSample(textures[in.texture_index], s_diffuse, in.tex_coords);
}
